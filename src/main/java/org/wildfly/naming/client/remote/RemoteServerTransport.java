...
final class RemoteServerTransport {
    ...
    RemoteServerTransport(final Channel channel, final int version, final MessageTracker messageTracker, final RemoteNamingService remoteNamingService) {
    ...
    void handleLookup(final MessageInputStream message, final int messageId, final int id, final boolean preserveLinks) throws IOException {
        ...
        try (MessageInputStream mis = message) {
            if (version == 1) {
                final int parameterType = mis.readUnsignedByte();
                ...
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    ...
                    if (preserveLinks) {
                        result = remoteNamingService.getLocalContext().lookupLink(name);
                    } else {
                        result = remoteNamingService.getLocalContext().lookup(name);
                    }
    ...
    void handleBind(final MessageInputStream message, final int messageId, final int id, final boolean rebind) throws IOException {
        try (MessageInputStream mis = message) {
            if (version == 1) {
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    int parameterType = mis.readUnsignedByte();
                    if (parameterType != Protocol.P_NAME) {
                        Messages.log.unexpectedParameterType(Protocol.P_NAME, parameterType);
                    }
                    final Name name = unmarshaller.readObject(Name.class);
        ...
        try (MessageInputStream mis = message) {
            if (version == 1) {
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    ...
                    remoteNamingService.getLocalContext().rename(oldName, newName);
                }
            } else {
        ...
        try (MessageInputStream mis = message) {
            if (version == 1) {
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    ...
                    final Name name = unmarshaller.readObject(Name.class);
                    result = remoteNamingService.getLocalContext().createSubcontext(name);
                } catch (ClassNotFoundException e) {
                    throw new IOException(e);
                }
            } else {
        ...
        try (MessageInputStream mis = message) {
            if (version == 1) {
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    ...
                    final Name name = unmarshaller.readObject(Name.class);
                    results = remoteNamingService.getLocalContext().list(name);
                } catch (ClassNotFoundException e) {
                    throw new IOException(e);
                }
            } else {
        ...
        try (MessageInputStream mis = message) {
            if (version == 1) {
                try (Unmarshaller unmarshaller = createUnmarshaller(mis, configuration)) {
                    ...
                    final Name name = unmarshaller.readObject(Name.class);
                    results = remoteNamingService.getLocalContext().listBindings(name);
                } catch (ClassNotFoundException e) {
                    throw new IOException(e);
                }
            } else {
