...
public class RemoteNamingService {
    ...
    public void start(final Endpoint endpoint) throws IOException {
        registration = endpoint.registerService(Constants.NAMING, new ChannelOpenListener(), OptionMap.EMPTY);
    }
        ...
        public void channelOpened(Channel channel) {
            ...
            try {
                channel.receiveMessage(new Channel.Receiver() {
                    public void handleMessage(Channel channel, MessageInputStream message) {
                        try (MessageInputStream mis = message) {
                            ...
                            if (! Arrays.equals(namingHeader, Constants.NAMING_BYTES)) {
                            ...
                            final RemoteServerTransport remoteServerTransport = new RemoteServerTransport(channel, version, messageTracker, RemoteNamingService.this);
                ...
                try (MessageOutputStream mos = messageTracker.openMessage()) {
                    mos.write(Constants.NAMING_BYTES);
